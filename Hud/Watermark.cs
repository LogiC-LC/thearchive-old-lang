...
namespace TheArchive.Features.Hud
{
    public class Watermark : Feature
    {
	    ...
	    [FSDisplayName("水印模式", Language.Chinese)]
	    ...
	    [FSDescription($"{nameof(WatermarkMode.Mod)}: 显示当前安装的模组版本\n{nameof(WatermarkMode.Positional)}: 显示当前位置 / XYZ 坐标", Language.Chinese)]
            public WatermarkMode Mode { get; set; } = WatermarkMode.Positional;

            [FSSlider(0f, 1f)]
            [FSDisplayName("XYZ Saturation")]
	    [FSDisplayName("XYZ 饱和度", Language.Chinese)]
            [FSDescription("How much color to apply to the XYZ coords.")]
	    [FSDescription("XYZ 坐标应用的颜色饱和度。", Language.Chinese)]
            public float XYZSaturation { get; set; } = 0.5f;

            [FSDisplayName("XYZ Decimals")]
	    [FSDisplayName("XYZ 小数位数", Language.Chinese)]
            [FSDescription("Decimal precision to show on the XYZ coords.")]
	    [FSDescription("在 XYZ 坐标上显示的小数精度", Language.Chinese)]

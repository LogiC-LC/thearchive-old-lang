...
namespace TheArchive.Features.Accessibility
{
    ...
    public class DynamicVolumeOverride : Feature
    {
        ...
        public class DynamicVolumeSettings
        {
            ...
            [FSDisplayName("音量覆盖", Language.Chinese)]
            [FSDescription("Volume percent to use during the games' intro and elevator ride sequences.")]
            [FSDescription("在游戏开头和电梯下降等场景中使用的音量百分比。", Language.Chinese)]
            ...
            [FSDescription($"设置切换窗口时音频的行为。\n\n<color=orange>{nameof(FocusChangeMode.Default)}</color> 使用游戏的实现\n<color=orange>{nameof(FocusChangeMode.MuteInBackground)}</color> 失去焦点时静音\n<color=orange>{nameof(FocusChangeMode.DoNothingInBackground)}</color> 失去焦点时保持音频\n<color=orange>{nameof(FocusChangeMode.OverrideInBackground)}</color> 失去焦点时使用覆盖音量", Language.Chinese)]
            ...
            [FSDisplayName("重置游戏音量", Language.Chinese)]
            ...
            [FSDescription("如果由于某种原因音频音量设置似乎没有恢复到默认值，尝试按下此按钮。", Language.Chinese)]
